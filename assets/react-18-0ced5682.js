import { r as reactExports, R as React } from "./index-c86cbf19.js";
...
}, renderElement = async (node, el2) => {
  let root = await getReactRoot(el2);
  return new Promise((resolve) => {
    root.render(React.createElement(WithCallback, { callback: () => resolve(null) }, node));
  });
}, unmountElement = (el2, shouldUseNewRootApi) => {
  let root = nodes.get(el2);
  root && (root.unmount(), nodes.delete(el2));
}, getReactRoot = async (el2) => {
  let root = nodes.get(el2);
  return root || (root = client.createRoot(el2), nodes.set(el2, root)), root;
};
export {
  ReactDOM__default as R,
